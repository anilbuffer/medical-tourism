...
export type CSTab = "triage_queues" | "raw_intake" | "master_cases" | "hospital_handovers" | "tele_consultations" | "quote_builder" | "visas_flights" | "concierge_hospitality" | "patient_messages" | "document_vault" | "consents_compliance" | "notes";
...
export const CSQueueView: React.FC<CSQueueViewProps> = ({
  ...
  const [internalTab, setInternalTab] = useState<CSTab>("triage_queues");
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300 h-full flex flex-col">
                      ...
                      filteredCases.map((c) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-4 align-top">
                              <div className="flex flex-col gap-1 items-start">
                                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${c.stage === "nurture" ? "bg-purple-100 text-purple-800"
      ...
      {activeTab === "master_cases" && (
        <div className="bg-white rounded-[24px] p-8 border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] min-h-[500px] flex flex-col items-center justify-center text-center">
          <Activity className="w-16 h-16 text-slate-200 mb-4" />
          <h2 className="text-2xl font-bold text-slate-900">Master Cases Directory</h2>
          <p className="text-slate-500 mt-2 max-w-md">Global search and full directory table goes here. Build out with filters for Market Region, Stage, and Coordinator.</p>
        </div>
      )}

      {/* Case Detail Inline View */}
      {!["triage_queues", "master_cases"].includes(activeTab) && (
        !activeCase ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 bg-white rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] min-h-[500px] p-8">
            <Activity className="w-16 h-16 text-slate-200 mb-4" />
            <h2 className="text-xl font-bold text-slate-900 mb-2">No Case Selected</h2>
            <p className="text-sm">Please go to Triage & SLA Queues and select a case first to view case details.</p>
          </div>
        ) : (
            ...
            <div className="flex-1">
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] min-h-[300px]">
                  ...
                  {/* ─── Tele-Consultations ───────────────────────────────────────── */}
                  {activeTab === "tele_consultations" && (
                    <div className="space-y-5 animate-in fade-in duration-300">
                      <h4 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                        <Video className="w-4 h-4 text-[#3F4EB4]" />
                        Tele-Consultation Scheduler & Timezone Matrix
                      </h4>
                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 text-center text-slate-500">
                        <p className="font-bold">Coming Soon</p>
                        <p className="text-xs mt-1">3-way scheduling between Patient, Hospital, and Coordinator.</p>
                      </div>
                    </div>
                  )}

                  {/* ─── Visas & Flights ───────────────────────────────────────── */}
                  {activeTab === "visas_flights" && (
                    <div className="space-y-5 animate-in fade-in duration-300">
                      <h4 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                        <Plane className="w-4 h-4 text-[#3F4EB4]" />
                        e-Medical Visa Desk & Flight Tracker
                      </h4>
                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 text-center text-slate-500">
                        <p className="font-bold">Coming Soon</p>
                        <p className="text-xs mt-1">Flight tracking and Visa document generation.</p>
                      </div>
                    </div>
                  )}

                  {/* ─── Concierge & Hospitality ───────────────────────────────────────── */}
                  {activeTab === "concierge_hospitality" && (
                    <div className="space-y-5 animate-in fade-in duration-300">
                      <h4 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-[#3F4EB4]" />
                        Concierge & Hospitality
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 text-center text-slate-500">
                        <p className="font-bold">Coming Soon</p>
                        <p className="text-xs mt-1">Airport reception, transfers, and hotel bookings.</p>
                      </div>
                    </div>
                  )}
