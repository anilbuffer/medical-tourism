      ...
      <div className="absolute inset-y-0 right-0 rtl:right-auto rtl:left-0 max-w-full flex">
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between border-l rtl:border-l-0 rtl:border-r border-slate-200 animate-in slide-in-from-right rtl:slide-in-from-left duration-300">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] bg-teal-500/20 text-teal-300 px-2 py-0.2 rounded-full font-bold">
                    Online
                  </span>
                </div>
          ...
          <div className="bg-teal-50/80 px-4 py-2 border-b border-teal-100 flex items-center justify-between text-[11px] text-teal-900">
            ...
            <span className="font-semibold text-teal-800">Replies in &lt; 15 mins</span>
          </div>
                ...
                >
                  {!isUser && (
                    <img
                      src={CARE_COORDINATOR.avatar}
                      alt="Coordinator"
                      className="w-7 h-7 rounded-full object-cover mb-1 ring-1 ring-teal-500 shrink-0"
                    />
                  )}
                  <div
            ...
            <div className="flex flex-wrap gap-1.5">
              {quickPrompts.map((q, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-teal-50 hover:text-teal-800 text-[11px] text-slate-700 font-medium transition-colors border border-slate-200 text-left rtl:text-right"
                >
                  {q}
                </button>
              ))}
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-200 flex items-center gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
            <button
              type="submit"
              disabled={!inputText.trim()}
              className="p-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 disabled:opacity-40 text-white shadow-md transition-all shrink-0"
            >
