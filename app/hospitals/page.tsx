            ...
            >
                ...
                <div className="relative h-64 w-full overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex flex-wrap gap-1.5">
                    {hosp.accreditations.map((acc, i) => (
                      <span
                        key={i}
                        className="px-2.5 py-1 rounded-full bg-slate-900/85 backdrop-blur-md border border-white/20 text-[#2ECDC5] text-[10px] font-bold flex items-center gap-1"
                      >
                        <ShieldCheck className="w-3 h-3 text-[#2ECDC5]" />
                        <span>{acc}</span>
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="absolute bottom-3 left-4 flex items-center gap-1 text-white text-xs font-semibold">
                    <MapPin className="w-3.5 h-3.5 text-[#2ECDC5]" />
                    <span>{language === "ar" ? hosp.cityAr : hosp.city}, {hosp.state}</span>
                  </div>
                </div>

                <div className="p-6 space-y-4">
                  <h3 className="text-2xl font-extrabold text-slate-900 group-hover:text-[#3F4EB4] transition-colors">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {hosp.specialties.map((spec, i) => (
                      <span
                        key={i}
                        className="px-2.5 py-1 rounded-lg bg-[#3F4EB4]/10 text-[#3F4EB4] text-xs font-semibold"
                      >
                        {spec}
                      </span>
                    ))}
              ...
              <div className="p-6 pt-0 space-y-3">
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#2ECDC5] via-[#3F4EB4] to-[#283593] hover:from-[#283593] hover:to-[#2ECDC5] text-white font-bold text-xs shadow-md shadow-[#283593]/20 transition-all flex items-center justify-center gap-2"
                >
